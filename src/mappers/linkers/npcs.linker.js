  ...
  return npcs
    .map((baseNpc) => {
      ...
      if (originalNpc.drops) {
        linkedNpc.drops = originalNpc.drops
          .map((drop) => {
      ...
      if (originalNpc.ware) {
        linkedNpc.ware = originalNpc.ware
          .map((w) => {
            ...
            linkedWare.chance = linkedWare.isGatcha
              ? linkedWare.rarity?.rate
              : null;
            return linkedWare;
          })
          .filter(Boolean);
      ...
      if (originalNpc.locations) {
        linkedNpc.locations = originalNpc.locations
          .map((key) => {
